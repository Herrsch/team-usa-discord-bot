...
The original purpose of the bot was just to send out a notification when someone starts streaming in the server's voice channel. This was mostly a selfish use case, as I'm not always looking at discord, and I didn't want to miss when my friends were hanging out. I think people ended up liking this feature though, as it is a very low-social-pressure way to signal to everyone else that they were down to hang, and everyone else doesn't feel pressured to join if they aren't free.
...
Boffos are a fake monopoly money that we earn and spend on our server. Every time we watch a movie together and add it to the scoreboard, each user that is in the voice call gets 10 boffos. Everyone's account balance is tracked in a #ledger channel, and is maintained by the bot. Users can send boffos to each other, or spend them on a few different things.
